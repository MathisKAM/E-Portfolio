...
// Approche simplifiée : juste attendre que tout soit chargé et forcer les animations
window.addEventListener('load', () => {
    console.log('🚀 Window loaded - Application simple des animations...');
    
    // Attendre 2 secondes pour être absolument sûr que tout est prêt
    setTimeout(() => {
        const activities = document.querySelectorAll('.activity');
        console.log(`🎯 Application directe sur ${activities.length} activités`);
        
        activities.forEach((el, index) => {
            const icon = el.querySelector('.activity-icon');
            const activityName = el.querySelector('p')?.textContent || `activité-${index}`;
            ...
            if (icon) {
                ...
                // Événements simples (pas de vérification, juste ajout)
                el.onmouseenter = function() {
                    this.querySelector('.activity-icon').style.transform = 'rotate(360deg) scale(1.1)';
                    console.log(`� HOVER START: ${activityName}`);
                };
                
                el.onmouseleave = function() {
                    this.querySelector('.activity-icon').style.transform = 'rotate(0deg) scale(1)';
                    console.log(`� HOVER END: ${activityName}`);
                };
                
                console.log(`✅ Animation simple appliquée: ${activityName}`);
            }
        });
        
        console.log('🎯 Animations appliquées de manière simple et directe');
    }, 2000);
});
